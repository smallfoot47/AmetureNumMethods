...
inline Coord2DMap Function_to_Map(std::function<double(double)> f, std::pair<double, double> interval);//converts a bounded function to a Coord2DMap, disregarding memory optimization
...
inline V_t Interpolate(V_t foot, V_t head, P_t percent)
{
	...
	//return foot * (1 - percent) + head * percent;
}
